...
public class Program {
    ...
    public static void main(String[] args) {
        /*
        Используя SQL, создать таблицу students с полями id (ключ), name и age.
        Реализация подключения к БД через JDBC:
        Напишите JAVA код для подключения к бд (н-р, MYSQL/PostgreSQL).
        Реализуйте вставку, чтение, обновление и удаление данных в таблице
        Students с использованием провайдера JDBC.
         */
...
//            System.out.println("Database connection close successfully");
    ...
    private static void useDatabase(Connection connection) throws SQLException {
        String createDBSQL = "USE studentsDB;";
        try (PreparedStatement statement = connection.prepareStatement(createDBSQL)) {
            statement.execute();
        }
    }
    ...
    private static void createTable(Connection connection) throws SQLException {
        String createTablrSQL = "CREATE TABLE IF NOT EXISTS students (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), age INT); ";
        try (PreparedStatement statement = connection.prepareStatement(createTablrSQL)) {
            statement.execute();
        }
    }
